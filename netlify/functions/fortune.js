...
然后输出完整版内容（包含：塑料袋的小心愿、今天最该回避的事、一句搞怪吉言）。风格要轻松、有梗。`;
        ...
        let ageRule = '';
        if (age && (age.includes('月') || parseInt(age) < 1)) ageRule = '年龄偏小（幼年）：建议包含磨牙、探索、短时间兴奋玩耍等';
        else if (age && (parseInt(age) >= 7 || age.includes('老'))) ageRule = '年龄偏大（老年）：建议包含软垫休息、温和散步、关节保健等';
        else if (age) ageRule = '成年宠物：建议适度运动、互动游戏、规律饮食';
        else ageRule = '年龄未知：正常通用内容';
